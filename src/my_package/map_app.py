...
from .paths import RESOURCES_DIR, DATA_DIR, FILE_DIR
...
"""
    ...
    def setup_web_channel(self):
        self.bridge = Bridge(self, self.file_dir)
    ...
    def load_map_html(self):
        try:
            ...
            base_url = QUrl.fromLocalFile(str(self.resources_dir) + "/")
    ...
    def add_point(self, lat, lng):
        ...
        self.dialog_window = DialogWindow(self, self.resources_dir, self.file_dir)
    ...
    def show_visible_area_dialog(self, bounds, current_zoom):
        ...
        info_label = QLabel(
            "Буду загружены тайлы для выбранных zoom уровней в пределах видимой области."
        )
    ...
    def start_download(self, bounds, zoom_levels, name, is_visible_area):
        ...
        progress_dialog.setWindowTitle(
            "Загрузка видимой области CartoDB Voyager"
            if is_visible_area
            else "Загрузка офлайн-карты CartoDB Voyager"
        )
        ...
        progress_label = QLabel(
            ...
            if is_visible_area
            else f"Загрузка тайлов CartoDB Voyager ({len(zoom_levels)} zoom слоев)..."
        )
    ...
    def on_download_progress(self, current, total, progress_bar, progress_label):
        if total > 0:
            ...
            progress_label.setText(
                f"Загружено: {current} из {total} тайлов ({progress}%)"
            )
    ...
    def on_download_finished(self, tiles_downloaded, progress_dialog, is_visible_area, zoom_levels):
        ...
        if tiles_downloaded > 0:
            if is_visible_area:
                message = (
                    f"Загружено {tiles_downloaded} тайлов видимой области ({len(zoom_levels)} zoom слоев)"
                )
            else:
                message = (
                    f"Загружено {tiles_downloaded} тайлов CartoDB Voyager ({len(zoom_levels)} zoom слоев)"
                )
    ...
    def show_offline_stats(self):
        ...
        QMessageBox.information(
            self, "Статистика офлайн-карт CartoDB Voyager", stats_text
        )
